...
import { Code, Code2, Monitor, Rocket, Send, X } from "lucide-react";
        ...
        <>
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
            {messages.map((m, i) => (
              ...
              >
                <div
                  className={`px-8 py-2.5 rounded-2xl text-sm leading-relaxed 
      ...
      <div className="flex-1 flex flex-col">
        <div className="h-14 px-4 flex justify-between items-center border-b border-white/10 bg-black/80">
          ...
          <div className="flex gap-2">
            ...
            to-purple-500 text-sm font-semibold hover:scale-105 transition cursor-pointer"
            >
              <Rocket size={14} />
              Deploy
            </button>
            <button className="p-2" onClick={() => setShowCode(true)}>
              <Code2 className="cursor-pointer" size={18} />
            </button>
            <button className="p-2" onClick={()=>setShowFullPreview(true)}>
              <Monitor className="cursor-pointer" size={18} />
            </button>
          </div>
        </div>
      ...
      <AnimatePresence>
        {showFullPreview && (
          ...
          >
            ...
            <button className="absolute top-4 right-4 p-2 bg-black/70 rounded-lg cursor-pointer" onClick={()=>setShowFullPreview(false)}><X/></button>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
